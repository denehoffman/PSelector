class Uniqueness:
    def __init__(self, name, config, particle_map, is_thrown=False):
        ...
        if self.cuts is not None:
            if isinstance(self.cuts, str):
                self.cuts = [self.cuts]
            assert isinstance(self.cuts, list), (
                f'Uniqueness cuts must be a list of cut names or a single string for one cut (error in {self.name})'
            )
    ...
    def fill_hists(self, indent=3, n_dir=-1):
        ...
        if self.thrown:
            ...
            outstring = (
                '    ' * indent + 'if(!dComboWrapper->Get_IsComboCut()) {\n'
    ...
    def fill_string(self, indent=2, n_dir=-1):
        ...
        if self.thrown:
            ...
            if self.particles[0] == 'Beam':
                outstring += (
                    '    ' * indent
                    + f'if(locUsedSoFar_{self.name}.find(loc{self.particles[0]}ID) == locUsedSoFar_{self.name}.end()) {{\n'
                )
            else:
                for step in self.particle_map:
                    for particle in step:
                        if particle.get('name') == self.particles[0]:
                            if particle.get('particle').charge != 0:
                                outstring += (
                                    '    ' * indent
                                    + f'if(locUsedSoFar_{self.name}.find(loc{self.particles[0]}TrackID)) == locUsedSoFar_{self.name}.end()) {{\n'
                                )
                            else:
                                outstring += (
                                    '    ' * indent
                                    + f'if(locUsedSoFar_{self.name}.find(loc{self.particles[0]}NeutralID)) == locUsedSoFar_{self.name}.end()) {{\n'
                                )
                        ...
                        for particle in step:
                            if particle.get('name') == tracked_particle:
                                if particle.get('particle').charge != 0:
                                    outstring += (
                                        '    ' * indent
                                        + f'locUsedThisCombo_{self.name}[PDGtoPType({particle.get("pid")})].insert(loc{tracked_particle}TrackID);\n'
                                    )
                                else:
                                    outstring += (
                                        '    ' * indent
                                        + f'locUsedThisCombo_{self.name}[PDGtoPType({particle.get("pid")})].insert(loc{tracked_particle}NeutralID);\n'
                                    )
            outstring += (
                '    ' * indent
                + f'if(locUsedSoFar_{self.name}.find(locUsedThisCombo_{self.name}) == locUsedSoFar_{self.name}.end()) {{\n'
            )
            outstring += self.fill_hists(n_dir=n_dir)
...
class Histogram:
    def __init__(self, name, config):
        ...
        assert ('xrange' in self.histogram_config.keys()) or self.sourced_x or self.has_destination, (
            f"No 'xrange' variable for histogram {name}"
        )
        assert ('xbins' in self.histogram_config.keys()) or self.sourced_x or self.has_destination, (
            f"No 'xbins' variable for histogram {name}"
        )
        if self.is2D:
            assert ('yrange' in self.histogram_config.keys()) or self.sourced_y or self.has_destination, (
                f"No 'yrange' variable for histogram {name}"
            )
            assert ('ybins' in self.histogram_config.keys()) or self.sourced_y or self.has_destination, (
                f"No 'ybins' variable for histogram {name}"
            )
        ...
        if not self.has_destination:
            if n_dir >= 0:
                ...
                if self.is2D:
                    return (
                        '    ' * indent
                        + f'dHist_{self.tag}{tag} = new TH2D("{self.tag}", "{self.title};{self.get_xlabel()};{self.get_ylabel()}", {self.get_x_param("xbins")}, {self.get_x_param("xrange")[0]}, {self.get_x_param("xrange")[1]}, {self.get_y_param("ybins")}, {self.get_y_param("yrange")[0]}, {self.get_y_param("yrange")[1]});\n'
                    )
                else:
                    return (
                        '    ' * indent
                        + f'dHist_{self.tag}{tag} = new TH1D("{self.tag}", "{self.title};{self.get_xlabel()};{self.get_ylabel()}", {self.get_x_param("xbins")}, {self.get_x_param("xrange")[0]}, {self.get_x_param("xrange")[1]});\n'
                    )
            else:
                if self.is2D:
                    return (
                        '    ' * indent
                        + f'dHist_{self.tag}{tag} = new TH2D("{self.tag}{tag}", "{self.title};{self.get_xlabel()};{self.get_ylabel()}", {self.get_x_param("xbins")}, {self.get_x_param("xrange")[0]}, {self.get_x_param("xrange")[1]}, {self.get_y_param("ybins")}, {self.get_y_param("yrange")[0]}, {self.get_y_param("yrange")[1]});\n'
                    )
                else:
                    return (
                        '    ' * indent
                        + f'dHist_{self.tag}{tag} = new TH1D("{self.tag}{tag}", "{self.title};{self.get_xlabel()};{self.get_ylabel()}", {self.get_x_param("xbins")}, {self.get_x_param("xrange")[0]}, {self.get_x_param("xrange")[1]});\n'
                    )
        else:
            return ''
    ...
    def fill_string(self, tag='', indent=1, n_dir=-1):
        ...
        if self.is2D:
            return (
                '    ' * indent
                + f'dHist_{self.tag}{tag}->Fill({self.get_x_param("x")}, {self.get_y_param("y")}, {self.weight});\n'
            )
        else:
...
class FlatBranch:
    ...
    def init_string(self, indent=1):
        ...
        if self.isArray:
            ...
            outstring += (
                '    ' * indent
                + f"""dFlatTreeInterface->Create_Branch_FundamentalArray<{dtype}>("{self.branch_config['name']}_{array_name}", "{self.num_name}");\n"""
            )
        elif self.branch_config['type'] == 'TString':
            dtype = self.branch_config['type']
            outstring += (
                '    ' * indent
                + f"""dFlatTreeInterface->Create_Branch_NoSplitTObject<TObjString>("{self.branch_config['name']}");\n"""
            )
        else:
            dtype = self.branch_config['type']
            outstring += (
                '    ' * indent
                + f"""dFlatTreeInterface->Create_Branch_Fundamental<{dtype}>("{self.branch_config['name']}");\n"""
            )
        return outstring
    ...
    def fill_string(self, indent=2):
        ...
        if self.isArray:
            ...
            for i, value in enumerate(array_values):
                outstring += (
                    '    ' * indent
                    + f"""dFlatTreeInterface->Fill_Fundamental<{dtype}>("{self.branch_config['name']}_{array_name}", {array_values[i]}, {i});\n"""
                )
        elif self.branch_config['type'] == 'TString':
            dtype = self.branch_config['type']
            ...
            outstring += (
                '    ' * indent
                + f"""dFlatTreeInterface->Fill_Fundamental<{dtype}>("{self.branch_config['name']}", {value});\n"""
            )
        return outstring
    ...
    def fill_num_string(self, indent=2):
        ...
        return (
            '    ' * indent
            + f"""dFlatTreeInterface->Fill_Fundamental<Int_t>("{self.num_name}", {len(array_values)});\n"""
        )
